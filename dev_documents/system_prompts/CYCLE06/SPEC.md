# CYCLE06 Specification: Forced Alignment and Diarization

## Summary
This cycle focuses on implementing Forced Alignment and Diarization. Cycle 06 builds `diarizer.py` fulfilling the `Diarizer` protocol. This integrates Pyannote.audio and Wav2vec2. The Pyannote pipeline must be explicitly configured with the `"exclusive": true` parameter to mitigate speaker confusion during Japanese backchanneling ('aizuchi'). The Wav2vec2 forced alignment step maps the text tokens from Cycle 05 to the speaker clusters. It outputs `SpeakerLabel` models appended to `TranscriptionSegment`s.
...
We follow TDD. First, we write the test asserting the exact required behavior. Then we implement the core logic for Forced Alignment and Diarization, ensuring cyclomatic complexity never exceeds 10.
