# CYCLE07 Specification: Pipeline Orchestration & Memory Management

## Summary
This cycle focuses on implementing Pipeline Orchestration & Memory Management. Cycle 07 builds the composition root, `pipeline.py`. This orchestrator uses Dependency Injection to instantiate the concrete classes built in Cycles 02-06. The paramount feature is the execution loop over `AudioChunk`s. Inside this loop, after transcription and diarization, the code MUST explicitly execute `gc.collect()` and `torch.cuda.empty_cache()`. Without these explicit calls, the chunking strategy from Cycle 03 is useless, and OOM will occur.
...
We follow TDD. First, we write the test asserting the exact required behavior. Then we implement the core logic for Pipeline Orchestration & Memory Management, ensuring cyclomatic complexity never exceeds 10.
