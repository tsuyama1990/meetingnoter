# CYCLE04 Specification: Voice Activity Detection (VAD)

## Summary
This cycle focuses on implementing Voice Activity Detection (VAD). Cycle 04 implements `vad.py` fulfilling the `SpeechDetector` protocol. This module wraps the Silero VAD model. The logic must read an `AudioChunk`, resample it to 16kHz using `torchaudio`, and execute the VAD inference. Crucially, the parameters must be hardcoded for Japanese nuances: `min_silence_duration_ms` must be set between 500-1000ms. The output must be a validated list of `SpeechSegment` Pydantic models.
...
We follow TDD. First, we write the test asserting the exact required behavior. Then we implement the core logic for Voice Activity Detection (VAD), ensuring cyclomatic complexity never exceeds 10.
