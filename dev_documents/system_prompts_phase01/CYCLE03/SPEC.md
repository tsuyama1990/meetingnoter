# CYCLE03 Specification: Audio Preprocessing (Chunking)

## Summary
This cycle focuses on implementing Audio Preprocessing (Chunking). Cycle 03 addresses the Pyannote OOM constraint by implementing `chunker.py`. This module fulfills the `AudioSplitter` protocol. It will utilize the `subprocess` module to invoke the `ffmpeg` binary. The logic must calculate the duration of the input file and issue FFmpeg slice commands to generate 20-minute `.wav` segments without re-encoding (`-c copy`). It must output a list of valid `AudioChunk` Pydantic models containing precise timestamp offsets.
...
We follow TDD. First, we write the test asserting the exact required behavior. Then we implement the core logic for Audio Preprocessing (Chunking), ensuring cyclomatic complexity never exceeds 10.
